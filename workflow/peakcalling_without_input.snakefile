...
#####################################################
### For the sample configuration file, just repeat the target column in the input column
### Then run the pipeline as usual, just change the snakefile (`-s` option).
#####################################################
...
if ((eval(str(config["paired_end"])) == True)):
    rule macs_callpeak_PE:
        ...
        shell:
            ...
            INPUT_ID=$(grep -w {params.sample} {params.sample_config_table} | cut -f 2)
            CALL_BROAD=$(grep -w {params.sample} {params.sample_config_table} | cut -f 3 | sed -e 's/\\(.*\\)/\\L\\1/' | uniq)
            ...
            --name {params.sample}.filtered.BAMPE > {log.err} 2> {log.out}
            """
else:
    rule phantom_SE:
        input:
        ...
        shell:
            ...
            ${{CONDA_PREFIX}}/bin/Rscript ${{CONDA_PREFIX}}/bin/run_spp.R -rf -c='{input.target_bam}' -savp -out={output.phantom} &> {log.out}
            """
    ...
    rule macs_callpeak_SE:
        ...
        shell:
            ...
            --extsize $(cat {input.phantom}) > {log.err} 2> {log.out}
            """
...
if ((eval(str(config["paired_end"])) == True)):
    rule MACS_peak_QC_PE:
        ...
        shell:
            ...
            # define peak file
            CALL_BROAD=$(grep -w {params.target} {params.sample_config_table} | cut -f 3 | sed -e 's/\\(.*\\)/\\L\\1/' | uniq)
