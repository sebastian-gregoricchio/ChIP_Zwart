...
if ((eval(str(config["paired_end"])) == True)):
    rule macs_callpeak_PE:
        ...
        shell:
            ...
            INPUT_ID=$(grep {params.sample} {params.sample_config_table} | cut -f 2)
            CALL_BROAD=$(grep {params.sample} {params.sample_config_table} | cut -f 3 | sed -e 's/\\(.*\\)/\\L\\1/' | uniq)
...
if ((eval(str(config["paired_end"])) == True)):
    rule MACS_peak_QC_PE:
        ...
        shell:
            ...
            # define peak file
            CALL_BROAD=$(grep {params.target} {params.sample_config_table} | cut -f 3 | sed -e 's/\\(.*\\)/\\L\\1/' | uniq)
